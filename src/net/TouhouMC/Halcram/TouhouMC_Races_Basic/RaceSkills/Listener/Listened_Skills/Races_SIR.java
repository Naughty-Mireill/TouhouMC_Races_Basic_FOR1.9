...
public class Races_SIR extends JavaPlugin {
	...
	public static void seirei_summon(final Player pl, final Plugin plugin){
		...
		if (type <= 8.0D){
			...
			pl.sendMessage(TouhouMC_Races_Basic.tmc_Races_pre + ChatColor.AQUA + "雪の霊を召喚した！");
		}else{
			int n = 0;
			while (n < 1){
			...
			pl.sendMessage(TouhouMC_Races_Basic.tmc_Races_pre + ChatColor.GOLD + "岩の霊を召喚した");
		}
	...
	public static void onnryou_never_vanish(Player pl, Plugin plugin, EntityDamageByEntityEvent event, int boost){
		...
		if (rand > 0.7D){
			...
			if ((event.getDamager() instanceof Player)){
				...
				dpl.sendMessage(TouhouMC_Races_Basic.tmc_Races_pre + ChatColor.DARK_RED + "消えたくない・・・っ");
